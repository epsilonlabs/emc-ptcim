 ...
 */
public class Com4jPtcimCollectionSelectOperation extends SelectOperation {
	...
	public Object execute(Object target, Variable iterator, Expression ast, IEolContext context,
		...
		if (!(target instanceof Com4jPtcimCollection)) {
			return super.execute(target, iterator, ast, context, returnOnFirstMatch);
		}
		try {
			this.context = context;
			this.iterator = iterator;
			return decomposeAST((Com4jPtcimCollection) target, ast,returnOnFirstMatch);
	...
	protected Collection<Com4jPtcimObject> decomposeAST(Com4jPtcimCollection target, Expression ast, boolean returnOnFirstMatch) throws Exception {
		...
		if (isOptimisable(ast)) {
			...
			return (Collection<Com4jPtcimObject>) ret;

		}
	}

	@SuppressWarnings("unchecked")
	private Collection<Com4jPtcimObject> optimisedExecution(Com4jPtcimCollection target, Expression ast, boolean returnOnFirstMatch) throws EolRuntimeException {
		...
		if (!"name".equals(attributename.toLowerCase())) {		// Name is the default Id
			return (Collection<Com4jPtcimObject>) super.execute(target, iterator, (Expression) ast, context, returnOnFirstMatch);
		}
